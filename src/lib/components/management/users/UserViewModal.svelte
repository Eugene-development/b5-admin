			...
			>
				...
				>
					...
					<div class="relative mt-4 flex flex-wrap gap-2">
						<span
							class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium {getUserStatus(
								user
							) === 'active'
						...
						>
							<span
								class="h-1.5 w-1.5 rounded-full {getUserStatus(user) === 'active'
									? 'bg-emerald-400'
									: 'bg-red-400'}"
							></span>
									...
									<div>
										...
										{#if user.email}
											<a
												...
												class="mt-1 block text-sm text-indigo-600 hover:text-indigo-500 dark:text-indigo-400"
											>
												{user.email}
											</a>
										{:else}
											...
											<div class="mt-2 space-y-1">
												{#each user.phones as phone}
													<div class="flex items-center gap-2">
														...
														{#if phone.is_primary}
															<span
																class="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400"
																>основной</span
															>
														{/if}
													</div>
							...
							{#if user.bio || user.website || user.address || user.birth_date}
								...
								>
									...
									<div class="space-y-4">
										...
										{#if user.birth_date}
											<div
												class={user.website || user.address
													? 'border-t border-gray-200 pt-4 dark:border-gray-700'
													: ''}
											>
												<p
										...
										{#if user.bio}
											<div
												class={user.website || user.address || user.birth_date
													? 'border-t border-gray-200 pt-4 dark:border-gray-700'
													: ''}
											>
												<p
												...
												<p
													class="mt-1 whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-300"
												>
													{user.bio}
												</p>
											</div>
