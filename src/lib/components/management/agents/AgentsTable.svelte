			...
			{:else}
				{#each users as agent, index (agent.id + '-' + agent.status + '-' + updateCounter)}
					...
					>
						...
						<td class="px-3 py-3 text-sm whitespace-nowrap" role="cell" headers="col-verified">
						...
						>
							{#if agent.userStatus}
								<span
									...
									style="background-color: {agent.userStatus.color}20; color: {agent.userStatus
										.color}"
								>
						...
						>
							<div class="flex items-center justify-center space-x-2">
								...
								<ActionButton
									...
									disabled={isLoading}
									{isLoading}
									ariaLabel="Удалить пользователя {agent.name || agent.email}"
									title="Удалить"
								/>
							</div>
						</td>
								...
								<tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
									...
									<td class="px-3 py-3 text-sm whitespace-nowrap">
										{#if agent.userStatus}
											<span
												class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium"
												style="background-color: {agent.userStatus.color}20; color: {agent
													.userStatus.color}"
											>
												{#if agent.userStatus.icon}
													<span class="mr-1">{agent.userStatus.icon}</span>
