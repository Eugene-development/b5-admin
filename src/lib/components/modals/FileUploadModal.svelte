					...
					<div class="space-y-5">
							...
							>
								...
								>
									...
									<p class="mt-4 text-sm text-gray-700 dark:text-gray-300">
										<span class="font-semibold text-{accentColor}-600 dark:text-{accentColor}-400"
											>Нажмите для выбора</span
										>
										или перетащите файл
									</p>
									<p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
										PDF, DOC, DOCX, XLS, XLSX, PNG, JPG, ZIP, RAR, 7Z, TAR, GZ (MAX. 10MB)
									</p>
									<input
										id="file-upload"
										type="file"
										...
										accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.zip,.rar,.7z,.tar,.gz,.tar.gz,.tgz"
										disabled={isLoading}
									/>
								</label>
							</div>
						...
						{#if selectedFile}
							...
							>
								<div class="flex items-center gap-3">
									<div
										class="flex h-10 w-10 items-center justify-center rounded-lg bg-{accentColor}-100 dark:bg-{accentColor}-900/30"
									>
										<svg
											class="h-5 w-5 text-{accentColor}-600 dark:text-{accentColor}-400"
											fill="none"
											stroke="currentColor"
											viewBox="0 0 24 24"
										>
				...
				>
					<div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
						...
						<button
							...
							class="inline-flex items-center justify-center gap-2 rounded-lg bg-{accentColor}-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-{accentColor}-500 focus:ring-2 focus:outline-none focus:ring-{accentColor}-600 focus:ring-offset-2 disabled:opacity-50 dark:focus:ring-offset-gray-900"
						>
							{#if isLoading}
