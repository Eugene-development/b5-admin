<script>
	/**
	 * Компонент для отображения и смены статуса заявки на выплату
	 * Feature: bonus-payments
	 * Requirements: 6.5, 6.6, 6.7
	 */
	import { updateBonusPaymentRequestStatus } from '$lib/api/bonusPayments.js';
	...
	let currentStatus = $derived(
		request?.status || { name: 'Не указан', code: null, color: '#6B7280' }
	);
	...
	async function handleStatusChange(newStatusCode) {
		...
		isUpdating = true;
		try {
			const result = await updateBonusPaymentRequestStatus(request.id, newStatusCode);
			addSuccessToast('Статус заявки обновлён');
			if (onStatusChange) {
	...
	function handleClickOutside(event) {
		if (showDropdown && !event.target.closest('.payment-status-dropdown')) {
			showDropdown = false;
		}
	}
...
<div class="payment-status-dropdown relative inline-block">
	...
	{:else}
		...
		<button
			...
			style="background-color: {currentStatus.color}20; color: {currentStatus.color}; --tw-ring-color: {currentStatus.color};"
		>
			{#if isUpdating}
				<svg
					class="mr-1 h-3 w-3 animate-spin shrink-0"
				...
				<div class="py-1">
					{#each statuses as status}
						<button
							...
							class="flex w-full items-center px-4 py-2 text-left text-sm {status.code === currentStatus.code
