<script>
	...
	let { isOpen = false, onSave = () => {}, onCancel = () => {}, isLoading = false, contracts = [], orders = [] } = $props();
	...
	$effect(() => {
		if (isOpen) {
			formData = {
				title: '', description: '', contract_id: '', order_id: '', responsible_person: '', guilty_party: '',
				planned_resolution_date: '', actual_resolution_date: '', priority: 'medium', status: 'open', resolution_notes: '', is_active: true
			};
		}
	});
	...
	<div class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
		...
		<div class="flex min-h-full items-center justify-center p-4">
			...
			>
				...
				<div class="relative overflow-hidden bg-gradient-to-r from-rose-500 via-pink-500 to-fuchsia-500 px-6 py-5">
					...
					<div class="relative flex items-start justify-between">
						<div class="flex items-center gap-3">
							...
							<div>
								<h2 class="text-xl font-bold text-white">Добавить рекламацию</h2>
								<p class="mt-0.5 text-sm text-rose-100">Создайте новую рекламацию</p>
							</div>
						</div>
				...
				<form onsubmit={handleSubmit} class="max-h-[calc(100vh-200px)] overflow-y-auto p-6">
					<div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
						...
						<div class="sm:col-span-2">
							<label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Название <span class="text-red-500">*</span></label>
							<input type="text" id="title" bind:value={formData.title} required disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white" placeholder="Краткое описание проблемы" />
						</div>
						...
						<!-- Contract -->
						<div>
							<label for="contract_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Контракт</label>
							<select id="contract_id" bind:value={formData.contract_id} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
						...
						<!-- Order -->
						<div>
							<label for="order_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Закупка</label>
							<select id="order_id" bind:value={formData.order_id} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
								<option value="">Не выбрано</option>
						...
						<!-- Responsible Person -->
						<div>
							<label for="responsible_person" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Ответственный</label>
							<input type="text" id="responsible_person" bind:value={formData.responsible_person} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white" placeholder="ФИО" />
						</div>

						<!-- Guilty Party -->
						<div>
							<label for="guilty_party" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Виновная сторона</label>
							<input type="text" id="guilty_party" bind:value={formData.guilty_party} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white" />
						</div>

						<!-- Priority -->
						<div>
							<label for="priority" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Приоритет</label>
							<select id="priority" bind:value={formData.priority} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
						...
						<div>
							...
							<select id="status" bind:value={formData.status} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
						...
						<div class="sm:col-span-2">
							<div class="flex items-center gap-3 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
								<input type="checkbox" id="is_active" bind:checked={formData.is_active} disabled={isLoading} class="h-4 w-4 rounded border-gray-300 text-rose-600 focus:ring-rose-500 dark:border-gray-600 dark:bg-gray-700" />
								<label for="is_active" class="text-sm font-medium text-gray-900 dark:text-white">Рекламация активна</label>
							</div>
						</div>
					</div>
				</form>
