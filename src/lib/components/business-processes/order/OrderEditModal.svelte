	...
	$effect(() => {
		if (isOpen && order) {
			...
			formData = {
				...
				curator_percentage: order.curator_percentage ?? '2',
							...
							<div class="mt-4 grid grid-cols-2 gap-4">
								<div>
									<label
										for="delivery-date"
										class="block text-sm font-medium text-gray-700 dark:text-gray-300"
										>Планируемая дата поставки</label
									>
									<input
										type="date"
										id="delivery-date"
								...
								<div>
									<label
										...
										class="block text-sm font-medium text-gray-700 dark:text-gray-300"
										>Фактическая дата поставки</label
									>
									<input
										type="date"
										...
										>
											<div class="mb-2 flex items-center justify-between">
												<span class="text-xs font-medium text-gray-500 dark:text-gray-400">
													Позиция #{index + 1}
													{#if position.isExisting}<span class="ml-1 text-gray-400"
															>(существующая)</span
														>{:else}<span class="ml-1 text-green-500">(новая)</span>{/if}
												</span>
												<button
													type="button"
