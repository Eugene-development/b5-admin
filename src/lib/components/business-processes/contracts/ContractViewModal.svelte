			...
			>
				...
				>
					...
					<div class="relative mt-4 flex flex-wrap gap-2">
						<span
							class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium {contract.is_active
								? 'bg-emerald-500/20 text-emerald-100'
								: 'bg-red-500/20 text-red-100'}"
						>
							<span
								class="h-1.5 w-1.5 rounded-full {contract.is_active
									? 'bg-emerald-400'
									: 'bg-red-400'}"
							></span>
				...
				<div class="max-h-[calc(100vh-280px)] overflow-y-auto p-6">
					<div class="grid gap-6 lg:grid-cols-2">
							...
							>
								...
								<div class="space-y-4">
									<div>
										...
										{/if}
										{#if contract.project?.client?.phones && contract.project.client.phones.length > 0}
											<p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
												<a
													href="tel:{contract.project.client.phones[0].value}"
													class="hover:text-indigo-600"
												>
													{formatPhone(contract.project.client.phones[0].value)}
												</a>
											</p>
										{/if}
									</div>
						...
						<div class="space-y-6">
							...
							{#if isAdmin}
								<div
									class="rounded-xl border-2 border-indigo-200 bg-gradient-to-br from-indigo-50 to-purple-50 p-4 dark:border-indigo-800 dark:from-indigo-900/30 dark:to-purple-900/30"
								>
									<h3
									...
									<div class="grid grid-cols-2 gap-3">
										...
										<div class="rounded-lg bg-emerald-100 p-3 dark:bg-emerald-900/40">
											...
											<p class="text-lg font-bold text-emerald-700 dark:text-emerald-300">
												{formatCurrency(calculatedAgentBonus())}
											</p>
										</div>
										<div class="rounded-lg bg-emerald-100 p-3 dark:bg-emerald-900/40">
											<p class="text-xs text-emerald-600 dark:text-emerald-400">Бонус куратора</p>
											<p class="text-lg font-bold text-emerald-700 dark:text-emerald-300">
												{formatCurrency(calculatedCuratorBonus())}
											</p>
										</div>
									</div>
