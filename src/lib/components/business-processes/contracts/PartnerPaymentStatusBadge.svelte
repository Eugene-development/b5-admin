<script>
	import { updateContractPartnerPaymentStatus } from '$lib/api/finances.js';
	...
	let currentStatus = $derived(
		contract?.partnerPaymentStatus || { code: 'pending', name: 'Ожидает оплаты' }
	);
	...
	async function handleStatusChange(newStatusCode) {
		...
		isUpdating = true;
		try {
			const result = await updateContractPartnerPaymentStatus(contract.id, newStatusCode);
