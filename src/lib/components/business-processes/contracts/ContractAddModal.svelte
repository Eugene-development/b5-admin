	...
	function handleSave() {
		if (onSave && !isLoading && isFormValid) {
			const contractData = {
				...
				agent_percentage: 0, // Значение по умолчанию
				curator_percentage: 0, // Значение по умолчанию
		...
		switch (field) {
			...
			case 'contract_date':
				if (!value) {
					...
					// Revalidate planned_completion_date if it exists
			...
			>
				...
				<div class="mb-6">
					...
					<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
						Создайте новый контракт между проектом и компанией
					</p>
				</div>
				...
				<form onsubmit={handleSubmit} class="space-y-6">
					...
					>
						...
						>
							...
							{/if}
							{isLoading ? 'Сохранение...' : 'Создать контракт'}
						</button>
