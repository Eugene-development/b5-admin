		...
		switch (field) {
			...
			case 'contract_amount':
				if (value !== '' && value !== null) {
					...
					if (isNaN(num) || num < 0) newErrors.contract_amount = 'Сумма должна быть положительным числом';
			...
			case 'curator_percentage':
				if (value !== '' && value !== null) {
					const num = parseFloat(value);
					if (isNaN(num) || num < 0 || num > 100) newErrors[field] = 'Процент должен быть от 0 до 100';
	...
	<div class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
		...
		<div class="flex min-h-full items-center justify-center p-4">
			...
			>
				...
				<div class="relative overflow-hidden bg-gradient-to-r from-indigo-600 via-indigo-500 to-purple-600 px-6 py-5">
					...
					<div class="relative flex items-start justify-between">
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<p class="mt-0.5 text-sm text-indigo-100">{contract.contract_number || 'Новый договор'}</p>
							</div>
						</div>
				...
				<form onsubmit={handleSubmit} class="max-h-[calc(100vh-200px)] overflow-y-auto p-6">
					<div class="grid gap-6 lg:grid-cols-2">
						...
						<div class="space-y-5">
							...
							<div>
								<label for="actual-date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Факт. завершение</label>
								<input type="date" id="actual-date" value={formData.actual_completion_date} oninput={(e) => handleInputChange('actual_completion_date', e.target.value)} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white" />
							</div>
						</div>
					</div>
					...
					{#if isAdmin}
						<div class="mt-6 rounded-xl border-2 border-indigo-200 bg-gradient-to-br from-indigo-50 to-purple-50 p-5 dark:border-indigo-800 dark:from-indigo-900/30 dark:to-purple-900/30">
							...
							<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
								<div>
									<label for="contract-amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Сумма (₽)</label>
									<input type="number" id="contract-amount" min="0" step="0.01" value={formData.contract_amount} oninput={(e) => handleInputChange('contract_amount', e.target.value)} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white" placeholder="0" />
								...
								<div>
									...
									<input type="number" id="agent-percentage" min="0" max="100" step="0.01" value={formData.agent_percentage} oninput={(e) => handleInputChange('agent_percentage', e.target.value)} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white" />
									{#if errors.agent_percentage}<p class="mt-1 text-sm text-red-600 dark:text-red-400">{errors.agent_percentage}</p>{/if}
								</div>
								<div>
									<label for="curator-percentage" class="block text-sm font-medium text-gray-700 dark:text-gray-300">% куратора</label>
									<input type="number" id="curator-percentage" min="0" max="100" step="0.01" value={formData.curator_percentage} oninput={(e) => handleInputChange('curator_percentage', e.target.value)} disabled={isLoading} class="mt-1.5 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-white" />
									{#if errors.curator_percentage}<p class="mt-1 text-sm text-red-600 dark:text-red-400">{errors.curator_percentage}</p>{/if}
								</div>
							</div>
