<script>
	...
	let formData = $state({
		id: '', project_id: '', description: '', comment: '', is_active: true, approval_status: 'none'
	});
	...
	$effect(() => {
		if (isOpen && !wasOpen && tz) {
			formData = {
				id: tz.id, project_id: tz.project_id || '', description: tz.description || '',
				comment: tz.comment || '', is_active: tz.is_active ?? true, approval_status: getApprovalStatus(tz)
			};
			errors = {};
		}
		wasOpen = isOpen;
	...
	function handleSubmit(event) {
		...
		const cleanedData = {
			id: formData.id, project_id: formData.project_id, description: formData.description.trim(),
			comment: formData.comment.trim() || null, is_active: formData.is_active,
				...
				<form onsubmit={handleSubmit} class="max-h-[calc(100vh-200px)] overflow-y-auto p-6">
					<div class="space-y-5">
						...
						<div class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
							<p class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Статус согласования</p>
							<div class="space-y-2">
								<label class="flex items-center gap-2">
									<input type="radio" name="edit_approval_status" bind:group={formData.approval_status} value="none" disabled={isLoading} class="h-4 w-4 border-gray-300 text-violet-600 focus:ring-violet-500 dark:border-gray-600" />
									<div>
										<span class="text-sm text-gray-700 dark:text-gray-300">Не требует согласования</span>
										<p class="text-xs text-gray-500 dark:text-gray-400">Техзадание не требует дополнительного одобрения</p>
									</div>
								</label>
								<label class="flex items-center gap-2">
									<input type="radio" name="edit_approval_status" bind:group={formData.approval_status} value="requires_approval" disabled={isLoading} class="h-4 w-4 border-gray-300 text-violet-600 focus:ring-violet-500 dark:border-gray-600" />
									<div>
										<span class="text-sm text-gray-700 dark:text-gray-300">Требуется согласование</span>
										<p class="text-xs text-gray-500 dark:text-gray-400">Техзадание ожидает одобрения перед началом работ</p>
									</div>
								</label>
								<label class="flex items-center gap-2">
									<input type="radio" name="edit_approval_status" bind:group={formData.approval_status} value="approved" disabled={isLoading} class="h-4 w-4 border-gray-300 text-violet-600 focus:ring-violet-500 dark:border-gray-600" />
									<div>
										<span class="text-sm text-gray-700 dark:text-gray-300">Согласовано</span>
										<p class="text-xs text-gray-500 dark:text-gray-400">Техзадание уже согласовано и готово к работе</p>
									</div>
								</label>
							</div>
						</div>
