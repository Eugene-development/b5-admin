<script>
	...
	$effect(() => {
		if (isOpen) {
			formData = { name: '', description: '', start: '', end: '', company_id: '', is_active: false };
			errors = {};
		}
	});
				...
				<div class="relative overflow-hidden bg-gradient-to-r from-amber-500 via-orange-500 to-yellow-500 px-6 py-5">
					...
					<div class="relative flex items-start justify-between">
						<div class="flex items-center gap-3">
							...
							<div>
								<h2 class="text-xl font-bold text-white">Добавить акцию</h2>
								<p class="mt-0.5 text-sm text-amber-100">Заполните информацию о новой акции</p>
							</div>
						</div>
