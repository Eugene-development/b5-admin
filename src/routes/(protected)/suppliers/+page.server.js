...
/**
 * Load suppliers from GraphQL API with streaming
 */
async function loadSuppliersData(fetch) {
	const query = `
		query GetCompanies {
	...
	try {
		...
		const rawSuppliers = suppliersData.map((company) => ({
			...company,
		...
		const suppliers = addSequentialNumbers(rawSuppliers);
...
export async function load({ fetch }) {
	...
	return {
		suppliersData: Promise.resolve({ suppliers: [] })
	};
}
