		...
		try {
			await retryOperation(
				async () => {
					...
					addSuccessToast(`Агент "${createdUser.name || createdUser.email}" создан.`);
				},
				2,
				1000
			);
			...
			await retryOperation(
				async () => {
					if (type === 'ban') {
						...
						addSuccessToast(`Агент "${user.name || user.email}" забанен.`);
					} else if (type === 'unban') {
						const result = await unbanUser(user.id);
						// Convert GraphQL enum to lowercase for consistency
						const status = result?.status?.toLowerCase() || 'active';
						updateUserStatus(user.id, status);
						addSuccessToast(`Агент "${user.name || user.email}" разбанен.`);
					} else if (type === 'delete') {
						await deleteUser(user.id);
						removeUserFromList(user.id);
						addSuccessToast(`Агент "${user.name || user.email}" удален.`);
					}
				},
				2,
				1000
