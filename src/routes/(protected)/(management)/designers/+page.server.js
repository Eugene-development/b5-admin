 ...
 */
async function loadDesignersData(token, fetch, hostname) {
...
export async function load({ locals, fetch, url }) {
	try {
		...
		// Load designers data with hostname for domain-based URL resolution
		const designersData = await loadDesignersData(locals.token, fetch, url.hostname);
