		...
		try {
			await retryOperation(
				async () => {
					...
					addSuccessToast(`Дизайнер "${createdUser.name || createdUser.email}" создан.`);
				},
				2,
				1000
			);
			...
			await retryOperation(
				async () => {
					if (type === 'ban') {
						...
						addSuccessToast(`Дизайнер "${user.name || user.email}" забанен.`);
					} else if (type === 'unban') {
						const result = await unbanUser(user.id);
						const status = result?.status?.toLowerCase() || 'active';
						updateUserStatus(user.id, status);
						addSuccessToast(`Дизайнер "${user.name || user.email}" разбанен.`);
					} else if (type === 'delete') {
						await deleteUser(user.id);
						removeUserFromList(user.id);
						addSuccessToast(`Дизайнер "${user.name || user.email}" удален.`);
					}
				},
				2,
				1000
...
<ProtectedRoute>
	{#snippet children()}
		<ErrorBoundary
			...
			showDetails={true}
		>
			<!-- Streamed Designers Data with SSR -->
						...
						<div class="px-4 py-7 sm:px-6 lg:px-7">
							<div class="mx-auto">
								<main id="main-content" aria-labelledby="page-title">
									...
									>
										<PageTitle title="Дизайнеры" />
										<div class="flex items-center space-x-3">
											<div class="w-80">
												<SearchBar
													bind:value={searchTerm}
													onSearch={handleSearch}
													placeholder="Поиск по таблице Дизайнеры..."
												/>
											</div>
											<!-- Add Button -->
									...
									{#if searchTerm.trim()}
										...
										>
											{#if filteredUsers.length === 0}
												Дизайнеры не найдены по запросу "{searchTerm}"
											{:else}
												Найдено {filteredUsers.length} дизайнер{filteredUsers.length === 1
													? ''
			...
			{:catch error}
				...
				<div class="flex min-h-screen items-center justify-center">
					<div class="rounded-lg border border-red-500/30 bg-red-500/20 p-8 text-center">
						<h3 class="mb-4 text-xl font-semibold text-white">Ошибка загрузки дизайнеров</h3>
						<p class="text-red-300">
							Не удалось загрузить дизайнеров. Попробуйте обновить страницу.
						</p>
					</div>
				</div>
			{/await}
