...
async function loadDeliveryData(fetch) {
	...
	try {
		...
		const deliveryData = result.data.companies.data.filter(
			(company) => company.status?.slug === 'delivery'
		);

		// Transform data to match expected format
		const rawDeliveryCompanies = deliveryData.map((company) => ({
			...company,
		...
		const deliveryCompanies = addSequentialNumbers(rawDeliveryCompanies);
