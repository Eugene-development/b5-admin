...
/**
 * Load contractors from GraphQL API with streaming
 */
async function loadContractorsData(fetch) {
	const query = `
		query GetCompanies {
	...
	try {
		...
		const contractorsData = result.data.companies.data.filter(
			(company) => company.status?.slug === 'contractors'
		);

		// Transform data to match expected format
		const rawContractors = contractorsData.map((company) => ({
			...company,
		...
		const contractors = addSequentialNumbers(rawContractors);
...
export async function load({ fetch }) {
	...
	return {
		contractorsData: Promise.resolve({ contractors: [] })
	};
}
